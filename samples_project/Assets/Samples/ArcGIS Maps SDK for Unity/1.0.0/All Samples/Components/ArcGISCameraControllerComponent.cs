	...
	public class ArcGISCameraControllerComponent : MonoBehaviour
	{
		...
		private void MoveCamera(double3 movDir)
		{
			...
			if (arcGISMapComponent.MapType == GameEngine.Map.ArcGISMapType.Global)
			{
				...
				{
					nextArcGISPoint.SetZ(MaxCameraHeight);
					cameraPosition = arcGISMapComponent.View.GeographicToWorld(nextArcGISPoint);
				}
				else if (nextArcGISPoint.Z < MinCameraHeight)
				{
					nextArcGISPoint.SetZ(MinCameraHeight);
					cameraPosition = arcGISMapComponent.View.GeographicToWorld(nextArcGISPoint);
				}
				else
				{
		...
		private void GlobalDragging(ref double3 cartesianPosition, ref quaternion cartesianRotation)
		{
			...
			if (isIntersected && intersection >= 0)
			{
				...
				lastArcGISPoint.SetX(currentGeoPosition.X + deltaX);
				lastArcGISPoint.SetY(currentGeoPosition.Y + deltaY);

				geoPosition.SetX(geoPosition.X + deltaX);
				var YVal = geoPosition.Y + (dotVC <= 0 ? -deltaY : deltaY);

				geoPosition.SetY(Math.Abs(YVal) < MaxCameraLatitude ? YVal : (YVal > 0 ? MaxCameraLatitude : -MaxCameraLatitude));
